  ...
  animations: [
    trigger('itemAnim', [
      ...
      transition('* => void', [
        ...
        // prevent from animating in at the same time, we need to implement a stagger animation

        // or another option would be completely prevent animating the list items on the initial load of the component.
      ])
    ]),
...
export class NotesListComponent implements OnInit {
  ...
  ngOnInit() {
    // we wanna retrieve all notes from NotesService
  ...
  filter(query: string) {
    ...
    // so first we must remove the duplicates

    let uniqueResults = this.removeDuplicates(allResults);
    this.filteredNotes = uniqueResults;

    // sorting search results by relevancy
  ...
  sortByRelevancy(searchResults: Note[]) {
    ...
    this.filteredNotes =  this.filteredNotes.sort((a: Note, b: Note) => {
      ...
      // if the return value is negative, then a is sorted before b;

      // if the return value is positive, then b is sorted before a;
